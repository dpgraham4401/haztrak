...
import {
  ...
  RcraSiteType,
  SiteType,
} from '~/components/Manifest/manifestSchema';
...
function getNextSigner(manifest: Partial<Manifest> | undefined):
  | {
      epaSiteId: string;
      siteType: SiteType;
...
export function ManifestForm({
  ...
  const setupSign = () => {
    ...
    const rcraSiteType = siteTypeToRcraSiteType(nextSigner?.siteType);
  ...
  const nextSigner = getNextSigner(manifestData);
    ...
    <Container className="mb-5">
      <ManifestContext.Provider
        value={{
          ...
          nextSigningSite: getNextSigner(manifestData),
